...
const AddTranslator = ({ onSuccess }) => {
  const { register, handleSubmit, formState: { errors } } = useForm();

  const onSubmit = async (data) => {
    try {
      await api.post('/translators', { full_name: data.full_name });
      toast.success('مترجم با موفقیت افزوده شد');
      onSuccess();
    } catch (err) {
      toast.error(err.response?.data?.error || 'خطا در افزودن مترجم');
    }
  };

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-6 rounded-xl shadow-sm max-w-full">
      <h2 className="text-xl font-bold text-gray-800 mb-6">افزودن مترجم</h2>
      <div className="mb-4">
